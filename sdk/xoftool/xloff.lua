...
local XLOFFRELOC_XR17032_HI16     = 6
local XLOFFRELOC_XR17032_LO16     = 7
local XLOFFRELOC_XR17032_LO16_1   = 8
local XLOFFRELOC_XR17032_LO16_2   = 9
...
archinfo[1].relocnames[XLOFFRELOC_XR17032_HI16]     = "HI16"
archinfo[1].relocnames[XLOFFRELOC_XR17032_LO16]     = "LO16"
archinfo[1].relocnames[XLOFFRELOC_XR17032_LO16_1]   = "LO16_1"
archinfo[1].relocnames[XLOFFRELOC_XR17032_LO16_2]   = "LO16_2"

archinfo[1].dofixup = function (tab, off, nval, rtype, tabvaddr)
	...
	elseif rtype == XLOFFRELOC_XR17032_FAR_LONG then
		...
		sv32(tab, off + 4, new2)
	elseif rtype == XLOFFRELOC_XR17032_HI16 then
		new = bor(band(nval, 0xFFFF0000), band(old, 0xFFFF))
	elseif rtype == XLOFFRELOC_XR17032_LO16 then
		new = bor(lshift(band(nval, 0xFFFF), 16), band(old, 0xFFFF))
	elseif rtype == XLOFFRELOC_XR17032_LO16_1 then
		new = bor(lshift(rshift(band(nval, 0xFFFF), 1), 16), band(old, 0xFFFF))
	elseif rtype == XLOFFRELOC_XR17032_LO16_2 then
		new = bor(lshift(rshift(band(nval, 0xFFFF), 2), 16), band(old, 0xFFFF))
	else
...
archinfo[1].dostub = function (section, ptr)
	...
	return stublocation, 4, XLOFFRELOC_XR17032_ABSJ, stublocation, 0
end
...
archinfo[2].dofixup = function (tab, off, nval, rtype, tabvaddr)
	if rtype == XLOFFRELOC_FOX32_LONG then
		-- nothing
	elseif (rtype == XLOFFRELOC_FOX32_SRC) or (rtype == XLOFFRELOC_FOX32_JMP) then
		off = off + 2
...
archinfo[2].dostub = function (section, ptr)
	...
	return stublocation, 6, XLOFFRELOC_FOX32_JMP, stublocation, 0
end
...
local XLOFFRELOC_I386_32 = 1
local XLOFFRELOC_I386_PC32 = 2
local XLOFFRELOC_I386_PLT32 = 4
local XLOFFRELOC_I386_16 = 20

archinfo[0x36383369] = {}
archinfo[0x36383369].name = "i386"
archinfo[0x36383369].align = 4
archinfo[0x36383369].id = 0x36383369

archinfo[0x36383369].relocnames = {}
archinfo[0x36383369].relocnames[XLOFFRELOC_I386_32] = "32"
archinfo[0x36383369].relocnames[XLOFFRELOC_I386_PC32] = "PC32"
archinfo[0x36383369].relocnames[XLOFFRELOC_I386_PLT32] = "PLT32"
archinfo[0x36383369].relocnames[XLOFFRELOC_I386_16] = "16"

archinfo[0x36383369].dofixup = function (tab, off, nval, rtype, tabvaddr)
	if rtype == XLOFFRELOC_I386_32 then
		sv32(tab, off, nval)
	elseif rtype == XLOFFRELOC_I386_PC32 then
		sv32(tab, off, nval - (tabvaddr + off))
	elseif rtype == XLOFFRELOC_I386_PLT32 then
		sv32(tab, off, nval - (tabvaddr + off))
	elseif rtype == XLOFFRELOC_I386_16 then
		sv16(tab, off, nval)
	else
		error("unknown relocation type "..rtype)
	end
...
archinfo[0x36383369].dostub = function (section, ptr)
	-- create a call stub template at the end of the given section.
	-- make sure to grow it.

	local stublocation = section.size
	section.size = section.size + 5

	section.data[stublocation] = 0xe9
	sv32(section.data, stublocation + 1, ptr - (section.vaddr + stublocation + 5))

	return stublocation, 5, XLOFFRELOC_I386_PC32, stublocation + 1, -4
end

archinfo[0x36383369].shouldredirect = function (section, fixup)
	-- determine if a fixup should be redirected to a call stub.
	return fixup.type == XLOFFRELOC_I386_PLT32
end
...
local XLOFFSECTIONFLAG_DEFERREDFREE = 32
...
function xloff.new(filename)
	...
	function img:write()
		...
		local function addRelocation(section, symbol, addend, offset, rtype, sectionindex)
			...
			for i = 0, self.sectioncount-1 do
				...
				for i,r in ipairs(section.relocs) do
					if (not r.dyresolved) and ((not r.symbol) or (r.symbol.added)) then
						if not addRelocation(section, r.symbol, r.addend, r.offset, r.type, r.section.id) then return false end
					end
				end
		...
		if not self.fstrip then
			for i = 0, self.importcount-1 do
				...
				for i,r in ipairs(import.fixups) do
					if not addRelocation(import, r.symbol, r.addend, r.offset, r.type, r.section.id) then return false end
				end
		...
		for k,v in ipairs(section.relocs) do
			...
			if sym then
				if sym.import and (not v.dyresolved) and movefixups then
					...
					self.arch.dofixup(section.data, v.offset, nval + v.addend, v.type, v.section.vaddr)
				end
			end
		end
	...
	function img:relocate()
		...
		for i = 0, self.importcount-1 do
			...
			for k,v in ipairs(import.fixups) do
				self.arch.dofixup(v.section.data, v.offset, v.symbol.value + v.addend, v.type, v.section.vaddr)
			end
		end
			...
			if sym.type == XLOFFSYMTYPE_EXTERN then
				...
				if lookup and ((lookup.type == XLOFFSYMTYPE_GLOBAL) or (lookup.type == XLOFFSYMTYPE_EXPORT)) then
					...
					if not self.nostubs then
						...
						local stublocation, stubsize, reloctype, reloclocation, relocaddend = self.arch.dostub(text, 0)
						...
						reloc.symbol = sym
						reloc.addend = relocaddend
						reloc.offset = reloclocation
